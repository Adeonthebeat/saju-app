...
INSTRUCTION_SUFFIX = """
각 항목은 아래 나열된 세부 질문에 전부 답하는 여러 문단(또는 줄바꿈으로 구분된
여러 항목)으로, 각 3~6문장 이상 충분히 구체적이고 길게 작성하세요. 문단을 나눌
때는 실제 줄바꿈만 쓰고, 줄바꿈을 나타내는 기호 문자(백슬래시+n 등)를 텍스트로
쓰지 마세요.
"""


class _CareerSection(BaseModel):
    career_fit: str  # 1) 타고난 직업·적성 (그릇 진단)
    career_timing: str  # 2) 타이밍과 이동수 (운의 흐름)


class _MoneySection(BaseModel):
    side_business: str  # 3) 사주 기반 부업·N잡
    wealth_building: str  # 4) 재물운과 자산 증식법


class _RelationshipSection(BaseModel):
    work_relationships: str  # 5) 일복과 직장 인간관계
    luck_improvement: str  # 6) 직업 개운법


_CAREER_INSTRUCTIONS = """
...
# (응답 스키마, 세부 질문 지시문) — 세 그룹으로 나눠 동시에 호출해 체감 응답
# 시간을 줄인다. 그룹당 항목 수·내용 길이는 그대로라 답변 품질은 동일하다.
_GROUPS: list[tuple[type[BaseModel], str]] = [
    (_CareerSection, _CAREER_INSTRUCTIONS),
    (_MoneySection, _MONEY_INSTRUCTIONS),
    (_RelationshipSection, _RELATIONSHIP_INSTRUCTIONS),
]
...
def _new_client() -> genai.Client:
    ...
    # (동시 3-way 호출로 재현됨). 클라이언트 생성 자체는 가벼워 비용이 작다.
